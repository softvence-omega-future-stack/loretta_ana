...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, onNavigate, currentView }) => {
  return (
    <>
      ...
      <aside className={`fixed lg:static top-0 left-0 h-screen bg-gray-900 text-white w-64 transform transition-transform duration-300 z-50 overflow-y-auto ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="p-4">
  <div className="flex items-center justify-between mb-4">
    <div className="px-3 py-2 rounded flex flex-col items-center">
      <img src="/image 6.png" alt="Logo" className="h-20" />
      <p className="text-sm text-[#FF7F27] mt-2">Created By Malinovic</p>
    </div>
        ...
        <nav className="px-3 pb-6">
          ...
          <div className="mb-4">
            ...
            {[
              ...
              <button key={item} className="w-full text-left px-3 py-2 rounded hover:bg-gray-800 mb-1 text-sm text-gray-300">
                {item}
              </button>
            ))}
          </div>
